...
For this study, two different methods with different approaches were selected in order to verify the effectiveness of the schreck. In order to compare both methods with each other, it is necessary to have the same initial situation and to use the same data. For this purpose, in a first step, a suitable schreck with a high coverage of different wild boar presence was selected on the basis of the introduced data sets. Afterwards, three wild boars were selected for this method comparison on the basis of the selected schreck. 
...
```
```{r, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="Trajectories from wild boar Caroline, Miriam and Frida"}

WSS_2016_01 <- schreck2 %>%
  filter(id == "WSS_2016_01")

# Option 1: With facet_wrap()
...
* **Methode (1):** The first approach compares the mean distance between wild boar and schreck-locations in the three different time periods. For this purpose, the distances of wild boar data points to the scare were calculated. These distances were afterwards statistically compared by using an one-way anova and a boxplot.

* **Methode (2):** The second approach analyzes the presence of wild boars by selected agricultural fields. Thereby, it is examined which wild boars were observed in the selected field before the activation of the scare and if and how often they were observed in the same field after the activation. Based on this analysis, a conclusion was  made by using as well an one-way anova and a boxplot. <br><br>For the second approach, the data package "Feldaufnahmen_Fanel" was additionally used, which subdivides the land cover resp. the type of agricultural area of the nature reserve Fanel. From the figure it can be concluded that the suitable schreck WSS_2016_01 is located on the grassland area No.2 (German: Wiese Nr. 2). Grassland areas are optimal for our research question, since wild boars spoil the ground during their search for food.
...
Der Boxplot sowie die statische Auswertung der Einfaktoriellen Anova haben ergeben, dass es sich hierbei um einen signifikanten Unterschied in der Mittleren Distanz zum Schreck WSS_20216_01 in den verschiedenen Modus gibt. Aufgrund dieser Methode ist bestätigt, dass der Schreck wirkung auf die Wildschweine zeigt. 
...
p1 <- ggplot(st_intersection(crop_fanel, WSS_2016_01_bbox, Caroline)) +
  geom_sf(aes(fill = Frucht)) +
...
p2<- ggplot(st_intersection(crop_fanel, WSS_2016_01_bbox, Miriam)) +
  geom_sf(aes(fill = Frucht)) +
  geom_sf(data = Miriam) +
  geom_sf(data = WSS_2016_01, color = "yellow") +
  geom_sf_text(aes(label = Frucht), size =1.2) +
...
```{r echo=FALSE, fig.show='hide', message=TRUE, warning=FALSE}

# ANOVA

Caroline_3 <- Caroline_2 %>%
  filter(Frucht == "Wiese")
...
# Caroline
a4 <- aov(perc~Schreck, data = Caroline_3)
summary(a4)

# Miriam
a5 <- aov(perc~Schreck, data = Miriam_3)
summary(a5)

# Frida
a6 <- aov(perc~Schreck, data = Frida_3)
summary(a6)

```


```{r, echo = FALSE, warning=FALSE, message=TRUE, fig.cap="Mean Distance to WSS_2016_01 Caroline, Miriam and Frida"}

# Reihenfolge einfügen 
bind_rows(Caroline_3, Miriam_3, Frida_3) %>%
  ggplot() +
  geom_boxplot(aes(Schreck, perc)) +
  facet_wrap(~Schreck) +
  labs(title = "Mean Perc to WSS_2016_01 on diffrent Modus")+
  xlab("Modus") + ylab("Perc")
...
-Säugetieratlas
- Forschungsprojekt: Prävention von Wildschweinschäden in der Landwirtschaft, Stefan Suter 2013 https://www.aramis.admin.ch/Default?DocumentID=3369&Load=true
